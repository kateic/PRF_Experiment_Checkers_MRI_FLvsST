...
class PRFStim(object):  
    def __init__(self, session, 
        ...
        self.bar_width_in_pixels = tools.monitorunittools.deg2pix(bar_width_deg, self.session.monitor) * self.tex_nr_pix / self.session.stim_size_in_pix
        ...
        if self.session.settings['PRF stimulus settings']['Flicker'] == True:
        #FLICKERING STIMULUS 
            #construct textures
            self.sqr_tex_1 = np.sign( np.sin(tex_x) * np.sin(tex_y) )
            self.sqr_tex_2 = -np.sign( np.sin(tex_x) * np.sin(tex_y) )
            ...
            self.sqr_tex_1[:, :bar_start_idx] = 0       
            self.sqr_tex_1[:, bar_end_idx:]   = 0
    
            self.sqr_tex_2[:, :bar_start_idx] = 0       
            self.sqr_tex_2[:, bar_end_idx:]   = 0
            ...
            self.checkerboard_1 = visual.GratingStim(self.session.win,
                                                     tex   = self.sqr_tex_1,
                                                     units ='pix',
                                                     size  = [self.session.stim_size_in_pix, self.session.stim_size_in_pix])
        ...
        #STANDARD MOVING STIMULUS 
            #construct textures
            if squares_in_bar == 1: #makes sure that single-square bar is centered in the middle, here redundant because always use 2 squares
    ...
    #this is the function that actually draws the stimulus. for the standard stimulus the sequence of different textures gives the illusion of motion.
    def draw(self, time, pos_in_ori, orientation,  bar_direction):
        
        #calculate position of the bar in relation to its orientation
        x_pos, y_pos = np.cos((2.0 * np.pi) * -orientation / 360.0) * pos_in_ori, \
                                np.sin((2.0 * np.pi) * -orientation / 360.0) * pos_in_ori

        
        if self.session.settings['PRF stimulus settings']['Flicker'] == True:
        # FLICKERING STIMULUS 
            #convert current time to sine/cosine to decide which texture to draw
            sin = np.sin(2 * np.pi * time * (self.flicker_frequency / 2))
            cos = np.cos(2 * np.pi * time * (self.flicker_frequency / 2))

            #set position, orientation, texture, and draw bar
